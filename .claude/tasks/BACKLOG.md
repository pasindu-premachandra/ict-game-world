...
| 1.2 | Adventure set, 18 mini games | ~~medium~~ **done 2026-09-20** | `data/adventure-9.json`, 6 themed sets, three kinds: `mcQuiz` 11, `sortGame` 4, `memoryGame` 3. Bonus content per gate D3 in `redesign-trilingual`, so not counted in the 112. Built and proven 2026-09-20: 27/27 play to 100 in both languages. Grade 9 got the 18 adventure games; grades 6-8 got 9 newly authored ones (`content/bonus-games.json`, validated by `scripts/lib/read-bonus.mjs`), because the adventure set is grade 9 content and there was nothing spare. A `Bonus games` button sits beside `Leaderboard` on every grade. The 9 new games are entirely drafted and need Ishini and Dilini. See `adventure-set/README.md`. |
